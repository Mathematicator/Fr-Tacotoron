...
from dtw import *
from Dtw_matrix import matrix_to_csv
...
def prepare_dataloaders(hparams):
    ...
    # print([i for i in iter(train_loader)])
    # for i, (text_padded, input_lengths, mel_padded, gate_padded, \
    #         output_lengths, basename, cleaned_text) in enumerate(train_loader):
    #     print("this is basename: {}".format(basename))
...
def validate(model, criterion, valset, iteration, batch_size, n_gpus,
    ...
    with torch.no_grad():
        ...
        log("len(val_loader): "+str(len(val_loader)))
...
def train(output_directory, log_directory, checkpoint_path, warm_start, n_gpus,
    ...
    for epoch in range(epoch_offset, hparams.epochs):
        ...
        # for i_train_loader, (text_padded, input_lengths, mel_padded, gate_padded, \
        # output_lengths, basenames, cleaned_text) in enumerate(train_loader):
        matrix = []
        for i_batch, batch in enumerate(train_loader):
            (text_padded, input_lengths, mel_padded, gate_padded, \
        output_lengths, basenames, cleaned_text) = batch
            ...
            output_dir_dtw=os.path.join(os.getcwd(), 'DTW_Train_output')
            ...
            for i_basename, basename in enumerate(basenames):
                ponctuation = cleaned_text[i_basename][0]
                if i_batch == 0:
                    special_iterator =  i_basename
                else:    
                    special_iterator = (i_basename-1)*10 + i_basename
                # print("this is basename: {}".format(basename))
                ...
                with open("FR-fr_Our/metadata_train_NEB_without_phonetics.csv", encoding="utf-8") as f:
                    for i_ligne , line in enumerate(f):
                        ...
                        if basename == parts[0] : 
                        # print ("continue punct: {}".format(text_original[0]))
                            text_original = parts[1]
                            break  


                # try : 
                #     print("special _iterator :{} basename :{} ".format(special_iterator, basename))
                #     if hparams.flag_predicted_saved == True:
                #         with open(os.path.join(output_dir_dtw,'Training_predicted_mel_frames_length_{}_{}_punc_{}.txt'.format(checkpoint_name , data_name, len(ponctuations))), "r", encoding="utf-8") as file_predicted:
                ...
                if hparams.flag_predicted == True and  hparams.flag_truncate :
                    with open(os.path.join(output_dir_dtw,'Training_predicted_mel_frames_length_{}_{}_punc_{}.txt'.format(checkpoint_name , data_name, len(ponctuations))), "a+", encoding="utf-8") as file_predicted:
                ...
                np.save(os.path.join(mel_dir, mel_filename), mel_outputs[i_basename].float().data.cpu().numpy()[0], allow_pickle=False)
                print("mel predicted file {} saved in {}".format(mel_filename, mel_dir))
            
                query_mel = mel_outputs[i_basename].float().data.cpu().numpy().T
                original_mel = melspec_original.float().data.cpu().numpy().T
                print("original shape {}".format(original_mel.shape))
                print("query shape {}".format(query_mel.shape))
                # distance, path = fastdtw(original_mel,query_mel,  dist=dist_MSE_BL)
                # norm_distance = distance / len(path)
                norm_distance = dist_MSE_BL(original_mel,query_mel)
        ...
        mtr = matrix_to_csv(ponctuations, matrix, hparams.training_files,  checkpoint_name, data_name)
        print(mtr)    
