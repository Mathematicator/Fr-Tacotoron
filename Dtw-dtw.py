if __name__ == '__main__':
    ...
    dataframe_name = os.path.join( dataframe_dir, "export_dataframe_dtw_checkpoint_79992.pt_test_data.csv.csv")
    df = pd.read_csv(dataframe_name)
    df['Original'] = df.Original.astype(str)
    df['Predict'] = df.Predict.astype(str)
    df['Score_dtw'] = df.Score_dtw.astype(float)
    ...
    mapper = LinearColorMapper(palette=colors, low=df.Score_dtw.min(), high=df.Score_dtw.max())
    ...
    x = list(np.reshape(H[:,:,0],(60)))
    y = list(np.reshape(H[:,:,1],(60)))
    ...
    p = figure(title="Categorical Heatmap",
                ...
                tooltips=[('couple', '@Original @Predict'), ('Score_dtw', '@Score_dtw%')])
    ...
    p.rect(x="Predict", y="Original", width=1, height=1,
        source=df,
        fill_color={'field': 'Score_dtw', 'transform': mapper},
        line_color=None)
